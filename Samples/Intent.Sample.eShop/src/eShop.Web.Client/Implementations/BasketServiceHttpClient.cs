...
{
    public class BasketServiceHttpClient : IBasketService
    {
        ...
        public async Task<BasketDto> GetBasketByIdAsync(string id, CancellationToken cancellationToken = default)
        {
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                {
                    return (await JsonSerializer.DeserializeAsync<BasketDto>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
